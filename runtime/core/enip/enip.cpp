...
// bytes.
//-----------------------------------------------------------------------------
int16_t enip_process_message(unsigned char *buffer, int16_t buffer_size, void* user_data)
{    
...
int enip_cfg_handler(void* user_data, const char* section,
                     const char* name, const char* value)
{
    if (strcmp("enip", section) != 0)
	{
        return 0;
    }

    auto config = reinterpret_cast<EnipConfig*>(user_data);
...
int8_t enip_run(oplc::config_stream& cfg_stream,
                        ...
                        volatile bool& run)
{
    ...
    ini_parse_stream(oplc::istream_fgets, cfg_stream.get(),
	                 enip_cfg_handler, &config);
    cfg_stream.reset(nullptr);

    // If we have a config override, then check for the port number
    config.port = strlen(cfg_overrides) > 0 ? atoi(cfg_overrides) : config.port;

    spdlog::info("Starting enip on port {}", config.port);
    start_server(config.port, run, &enip_process_message, nullptr);
                      ...
                      volatile bool& run, const char* config)
{
    ...
    enip_run(cfg_stream, config, binding, run);
}
